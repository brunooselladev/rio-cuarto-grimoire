            ...
            <Card className="border-accent/50 border-glow-cyan animate-fade-in">
              <CardHeader>
                <CardTitle className="text-accent font-mono">
                  {editingPOI ? 'Editar Mi Ubicación' : 'Sugerir Nueva Ubicación'}
                </CardTitle>
                <CardDescription className="font-mono">
                  {editingPOI ? 'Modifica los detalles de tu punto de interés.' : 'Propón un nuevo punto de interés para la campaña.'}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <Label className="font-mono text-xs">NOMBRE</Label>
                  <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ej: El Café del Artista" className="font-mono" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="¿Qué tiene de especial este lugar?" className="font-mono" />
                </div>
                 <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground font-mono">{form.images.length} imagen(es)</span>
                      </div>
                      {form.images.length > 0 && (
                        <div className="grid grid-cols-3 gap-2">
                          {form.images.map((img, idx) => (
                            <div key={idx} className="relative group">
                              <img src={img} alt={`Preview ${idx + 1}`} className="w-full h-20 object-cover rounded border border-primary/30" />
                              <button onClick={() => removeImage(idx)} className="absolute top-1 right-1 bg-destructive text-destructive-foreground rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <X size={12} />
                              </button>
                            </div>
                          ))}
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="outline" onClick={() => { setShowNewPOI(false); setEditingPOI(null); }} className="font-mono">
                    CANCELAR
                  </Button>
                </div>
              </CardContent>
