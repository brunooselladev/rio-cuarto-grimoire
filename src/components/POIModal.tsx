...
const getTypeColor = (type: POI['type']) => {
  switch(type) {
    case 'power': return 'text-primary border-primary';
    case 'mission': return 'text-accent border-accent';
    case 'refuge': return 'text-secondary border-secondary';
    case 'danger': return 'text-destructive border-destructive';
...
const getTypeLabel = (type: POI['type']) => {
  switch(type) {
    case 'power': return 'PUNTO DE PODER';
    case 'mission': return 'MISIÓN ACTIVA';
    case 'refuge': return 'REFUGIO';
    case 'danger': return 'ZONA PELIGROSA';
    default: return 'UBICACIÓN';
  }
};

const POIModal = ({ poi, onClose }: POIModalProps) => {
      ...
      <div className="relative bg-card border-2 border-primary max-w-2xl w-full max-h-[80vh] overflow-y-auto animate-fade-in border-glow-green">
        ...
        <div className="sticky top-0 bg-card/95 backdrop-blur border-b border-primary/30 p-6 z-10">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <h2 className="text-2xl md:text-3xl font-bold glow-text-green mb-2">
                {poi.name}
              </h2>
              {poi.sphere && (
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-foreground leading-relaxed pl-6 border-l-2 border-primary/30">
              {poi.description}
            </p>
          </div>
          ...
          <div className="border-t border-border pt-4">
            <div className="text-xs font-mono text-muted-foreground space-y-1">
              <div>COORDENADAS: {poi.lat.toFixed(4)}, {poi.lng.toFixed(4)}</div>
              <div>ID: #{poi.id.toString().padStart(4, '0')}</div>
          ...
          <div className="flex gap-3 pt-4 border-t border-border">
            ...
            <Button 
              variant="outline"
              className="flex-1 border-secondary text-secondary hover:bg-secondary hover:text-secondary-foreground font-mono"
            >
